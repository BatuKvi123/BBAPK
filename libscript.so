...
function showAndroidToast(text) {
    Java.perform(function () {
        var contextClass = Java.use('android.app.ActivityThread');
        var currentApp = contextClass.currentApplication();
        var context = currentApp.getApplicationContext();

        var Toast = Java.use('android.widget.Toast');
        var CharSequence = Java.use('java.lang.String');

        Toast.makeText(context, CharSequence.$new(text), Toast.LENGTH_SHORT.value).show();
    });
}
...
function setupMessaging() {
    ...
    function onReceive() {
        ...
        if (messageType >= 10101) {
            ...
            if (messageType === 10101) {
                createDebugButton(); // this one is actually 22228
            }
    ...
    // Read string: [length (int32)][string bytes]
    var strlen = Memory.readS32(messageBuffer);
    var toastText = Memory.readUtf8String(messageBuffer.add(4), strlen);

    showAndroidToast(toastText);
    free(messageBuffer);
    return;
}
...
Interceptor.attach(Module.findExportByName('libc.so', 'connect'), {
    ...
    if (ntohs(Memory.readU16(args[1].add(2))) === 9339) {
        ...
        var host = Memory.allocUtf8String("20.195.24.81");
        Memory.writeInt(args[1].add(4), inet_addr(host));
        Memory.writeU16(args[1].add(2), ntohs(parseInt(24581)));
        setupMessaging();
        }
    }
});
